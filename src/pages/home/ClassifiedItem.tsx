...
const ClassifiedItem: React.FC<ClassifiedItemProps> = ({ ad }) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'sold':
        return { color: 'bg-blue-100 text-blue-800', text: 'Sold' };
      case 'removed':
        return { color: 'bg-red-100 text-red-800', text: 'Removed' };
      default:
  ...
  return (
    <div 
      className={`rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-md transition duration-300 ${ad.isPriority ? 'ring-2 ring-pink-500 relative' : ''} cursor-pointer relative`}
      onClick={() => navigate(`/ad/${ad.id}`)}
    >
      {/* Priority badge */}
      {ad.isPriority && (
        <div className="absolute top-2 right-2 z-20 bg-gradient-to-r from-pink-500 to-blue-500 text-white text-xs font-medium px-2 py-1 rounded-full">
          Priority
        </div>
      )}
      ...
      <div className="p-4">
        <div className="flex justify-between items-start mb-2">
          <h3 className="font-semibold text-gray-800 line-clamp-1">{ad.title}</h3>
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          <span className={`text-xs px-2 py-1 rounded-full ${getConditionColor(ad.condition)}`}>
            {ad.condition}
          </span>
          ...
          {ad.status !== 'available' && (
            <span className={`text-xs px-2 py-1 rounded-full ${getStatusBadge(ad.status).color}`}>
