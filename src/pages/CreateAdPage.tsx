...
const CreateAdPage: React.FC = () => {
  accessElf.track('Create Advertisement Page')
  const navigate = useNavigate();
  const { categories, addNewAd } = useClassified();
  const [formData, setFormData] = useState({
    ...
    images: [] as string[],
    priority: false,
    status: 'available' as ClassifiedAd['status']
  });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    
    const newAd: Omit<ClassifiedAd, 'id' | 'posted_date' | 'sellerName'> = {
      ...formData,
  ...
  return (
    <div className="pt-24 pb-12 px-4">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-3xl font-bold mb-8">Create New Advertisement</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="e.g., Baby Stroller - Excellent Condition"
            />
          </div>
